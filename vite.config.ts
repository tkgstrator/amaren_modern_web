...
export default defineConfig({
  server: {
    ...
    proxy: {
      '/api': {
        target: 'http://amaren.e5.valueserver.jp/Rsys/php/amalen.fetchAllData.php', // 実際のAPIサーバーのURL
        changeOrigin: true, // オリジンをターゲットに合わせる
        rewrite: (path) => path.replace(/^\/api/, '') // パスの書き換え（必要に応じて）
      }
    }
  },
  define: {
